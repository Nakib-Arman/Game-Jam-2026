...
header_bottom_gap = 15  # extra space below headers
top_padding = 20         # space above first line of text
bottom_padding = 20      # space below last line of text

def draw_how_to_play(mouse, scroll_delta=0):
    """
    Draws a scrollable "How To Play" panel with a BACK button.
    Handles text wrapping, headers, and prevents overlapping by clipping.
    """
    ...
    visible_height = panel_height - 50 - top_padding - bottom_padding
    text_surface = pygame.Surface((panel_width - 40, visible_height))
    text_surface.fill((35, 35, 35))
    text_surface.set_colorkey((0, 0, 0))  # transparency key
    ...
    if total_text_height > visible_height:
        max_scroll = visible_height - total_text_height
    else:
        max_scroll = 0

    how_to_scroll = max(min(how_to_scroll, 0), max_scroll)

    # ---------- BLIT CLIPPED TEXT SURFACE ----------
    screen.blit(text_surface, (panel_x + 20, panel_y + 50), area=pygame.Rect(0, 0, panel_width - 40, visible_height))
