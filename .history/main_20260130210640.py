...
header_bottom_gap = 10  # smaller extra space below headers
...
def draw_how_to_play(mouse, scroll_delta=0):
    ...
    content_height = panel_height - 50
    text_surface = pygame.Surface((content_width, content_height), pygame.SRCALPHA)
    text_surface.fill((0, 0, 0, 0))  # transparent

    y_offset = top_padding + how_to_scroll
    line_spacing = 24  # smaller spacing
    total_text_height = 0
    ...
    for line in HOW_TO_PLAY_TEXT:
        ...
        # extra gap below headers
        if is_header:
            y_offset += header_bottom_gap
            total_text_height += header_bottom_gap

    # ---------- CALCULATE MAX SCROLL ----------
    visible_text_height = content_height - top_padding - bottom_padding
    max_scroll = min(0, visible_text_height - total_text_height)
    max_scroll = max(max_scroll, -200)  # shrink scroll range to max -200 px
    how_to_scroll = max(min(how_to_scroll, 0), max_scroll)

    # ---------- BLIT TEXT SURFACE WITH CLIPPING ----------
    screen.blit(text_surface, (panel_x + 20, panel_y + 50),
                area=pygame.Rect(0, 0, content_width, content_height))
