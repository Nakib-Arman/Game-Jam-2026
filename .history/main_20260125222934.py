...
# LIGHT SYSTEM
MAX_LIGHT = 100.0
LIGHT_DRAIN_RATE = 0.02  # tweak speed
MIN_LIGHT_RADIUS = 60    # minimum visible radius
...
def get_light_radius():
    max_radius = SCREEN_HEIGHT  # full height
    return int(MIN_LIGHT_RADIUS + (light_percentage / MAX_LIGHT) * (max_radius - MIN_LIGHT_RADIUS))
...
def draw_world(surface, radius=None):
    ...
    for row in range(VIEW_ROWS):
        for col in range(VIEW_COLS):
            ...
            # Only draw if inside circle (spotlight)
            if radius is not None:
                dx = screen_x + BASE_CELL_SIZE // 2 - center_x
                dy = screen_y + BASE_CELL_SIZE // 2 - center_y
                dist = (dx ** 2 + dy ** 2) ** 0.5
                if dist > radius:
                    continue
    ...
    pygame.draw.circle(surface, RED, (center_x, center_y), player_radius)
...
while running:
    ...
    if show_map:
        draw_map(screen, cave, (player_x, player_y), (SCREEN_WIDTH, SCREEN_HEIGHT))
    else:
        ...
        draw_world(mask_surface, radius=radius)
