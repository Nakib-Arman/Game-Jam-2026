...
def draw_how_to_play(mouse, scroll_delta=0):
    ...
    # ---------- TEXT ----------
    y_offset = panel_y + 50 + top_padding + how_to_scroll  # start below panel title with top padding
    line_spacing = 28
    max_text_width = panel_width - 40
    total_text_height = 0

    # store positions of lines to calculate total text height correctly
    lines_positions = []
    ...
    for line in HOW_TO_PLAY_TEXT:
        ...
        for word in words:
            ...
            if test_surf.get_width() > max_text_width:
                lines_positions.append((current_line, y_offset, font))
        ...
        # add extra gap below headers
        if is_header:
            y_offset += header_bottom_gap
            total_text_height += header_bottom_gap

    # ---------- DRAW LINES ----------
    for text, y, font in lines_positions:
        screen.blit(font.render(text, True, (255, 255, 255)), (panel_x + 20, y))

    # ---------- CALCULATE MAX SCROLL ----------
    visible_text_height = panel_height - 50 - top_padding - bottom_padding
    if total_text_height > visible_text_height:
        max_scroll = total_text_height - visible_text_height
    else:
        max_scroll = 0

    # clamp scroll (negative scroll, so top is 0)
