...
HOW_TO_PLAY_TEXT = [
    "🎯 OBJECTIVE",
    "Escape the cave by reaching the exit. Explore carefully, collect resources, manage your energy and light, and survive the monsters lurking in the darkness.",
    "",

    "🗺️ ITEMS YOU CAN FIND",
    "Map",
    "- Adds a map to your inventory.",
    "- Using a map reveals the cave layout and rearranges the doors, opening new paths.",
...
def draw_how_to_play(mouse, scroll_delta=0):
    ...
    pygame.draw.rect(screen, (32, 32, 32), panel_rect, border_radius=10)
    pygame.draw.rect(screen, (170, 170, 170), panel_rect, 2, border_radius=10)
    ...
    line_height = 30
    section_gap = 18        # gap after blank lines
    header_top_gap = 220     # <<< NEW GAP BEFORE HEADERS

    y = text_rect.y + how_to_scroll
    total_height = 0
    first_header = True

    # ---------- DRAW TEXT ----------
    for line in HOW_TO_PLAY_TEXT:
        is_header = line.isupper()

        if is_header:
            if not first_header:
                y += header_top_gap
                total_height += header_top_gap
            first_header = False

        font = font_title if is_header else font_text
        color = (240, 240, 240) if is_header else (210, 210, 210)
        ...
        for word in words:
            ...
            if font.render(test, True, color).get_width() > max_text_width:
                screen.blit(font.render(current_line, True, color), (text_rect.x, y))
                y += line_height
    ...
    # ---------- SCROLL LIMIT ----------
    min_scroll = min(0, text_rect.height - total_height - 10)
    how_to_scroll = max(min(how_to_scroll, 0), min_scroll)
    ...
    btn_text = font_small.render("BACK", True, (255, 255, 255))
