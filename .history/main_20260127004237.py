    ...
    for ox in (-player_radius, player_radius):
        for oy in (-player_radius, player_radius):
            ...
            if not (0 <= cx < WORLD_COLS and 0 <= cy < WORLD_ROWS):
                return False
            if cave[cy][cx] == WALL or cave[cy][cx] == GATE_CLOSED:
                return False
    return True
    ...
    for r in range(VIEW_ROWS):
        for c in range(VIEW_COLS):
            ...
            if tile == WALL:
                ...
                pygame.draw.rect(screen, (0, 255, 0), (sx, sy, BASE_CELL_SIZE, BASE_CELL_SIZE))
            elif tile == GATE_CLOSED:
                pygame.draw.rect(screen, (255, 0, 0), (sx, sy, BASE_CELL_SIZE, BASE_CELL_SIZE))
            else:
...
while running:
    ...
    for event in pygame.event.get():
        ...
        # For brevity, other menu / button logic unchanged
