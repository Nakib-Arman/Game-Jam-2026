...
def draw_how_to_play(mouse, scroll_delta=0):
    ...
    top_margin = panel_y + 30
    max_text_width = panel_width - 80

    line_height = 30
    section_gap = 14

    y = top_margin + how_to_scroll
    total_height = 0
    ...
    for line in HOW_TO_PLAY_TEXT:
        ...
        for word in words:
            ...
            if font.render(test, True, color).get_width() > max_text_width:
                screen.blit(font.render(current_line, True, color), (left_margin, y))
                y += line_height
    ...
    # ---------- SCROLL LIMIT ----------
    visible_height = panel_height - 40
    min_scroll = min(0, visible_height - total_height - 20)
